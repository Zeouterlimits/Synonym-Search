...
public class SynonymSearchPage extends DialogPage implements ISearchPage  //TextSearchPage
{
	...
	protected String[] previousExtensions;
	
	protected Button isRegExCheckbox;
	...
	private ArrayList fPreviousSearchPatterns = new ArrayList(HISTORY_SIZE);
	...
	private ISearchQuery newQuery(String synonym) throws CoreException {
		SearchPatternData data = getPatternData();
...
//	}
	...
	private SearchPatternData getPatternData() {
		SearchPatternData match = findInPrevious(searchText.getText());
		if (match != null) {
	...
	private String[] getPreviousExtensionsOldStyle() {
		ArrayList extensions= new ArrayList(fPreviousSearchPatterns.size());
...
//				}
	...
	private boolean validateRegex() {
		if (fIsRegExCheckbox.getSelection()) {
			try {
				PatternConstructor.createPattern(searchText.getText(), fIsCaseSensitive, true);
	...
	private void addTextPatternControls(Composite group) {
		...
		// Pattern combo
		searchText= new Combo(group, SWT.SINGLE | SWT.BORDER);
		...
		searchText.setFont(group.getFont());
	...
	private void handleWidgetSelected() {
		int selectionIndex= searchText.getSelectionIndex();
		...
		if (!searchText.getText().equals(patternData.textPattern))
			return;
		...
		fIsWholeWordCheckbox.setSelection(fIsWholeWord);
		fIsWholeWordCheckbox.setEnabled(!fIsRegExSearch);
		searchText.setText(patternData.textPattern);
		fPatterFieldContentAssist.setEnabled(fIsRegExSearch);
	...
	private boolean initializePatternControl() {
		...
		if (selection instanceof ITextSelection && !selection.isEmpty() && ((ITextSelection)selection).getLength() > 0) {
			...
			if (text != null) {
				if (fIsRegExSearch)
					searchText.setText(FindReplaceDocumentAdapter.escapeForRegExPattern(text));
				else
					searchText.setText(insertEscapeChars(text));

				if (fPreviousExtensions.length > 0) {
					fExtensions.setText(fPreviousExtensions[0]);
				} else {
					//TODO ui workbench?
					String extension = null; //= getExtensionFromEditor();
					if (extension != null)
